...
class FormPlace
{
	...
	*/
    ...
    function selectrooms($fk_place,$selected='',$htmlname='fk_resource_room',$showempty=0,$exclude='',$limitto='',$showfunction=0, $moreclass='', $options_only=false, $showbuilding=0, $forcecombo=0, $event=array())
    {
    	...
    	$sql.= " WHERE s.entity IN (".getEntity('resource', 1).")";
    ...
    /**
     *	Return list of all buildings (for a place or all)
     *
     *	@param	int		$fk_place      	Id of place or 0 for all
     *	@param  string	$selected   	Id room pre-selectionne
     *	@param  string	$htmlname  	    Name of HTML field ('none' for a not editable field)
     ...
     *	@return	 int						<0 if KO, Nb of contact in list if OK
     */
    function selectbuildings($fk_place,$selected='',$htmlname='fk_resource_building',$showempty=0,$exclude='',$limitto='',$showfunction=0, $moreclass='', $options_only=false, $showbuilding=0, $forcecombo=0, $event=array())
    {
    	global $conf,$langs;
    	...
    	// On recherche les salles
    	$sql = "SELECT s.rowid, s.ref, s.label";
    	$sql.= " FROM ".MAIN_DB_PREFIX ."place_building as s";
    	$sql.= " INNER JOIN  ".MAIN_DB_PREFIX ."place as p ON p.rowid=s.fk_place ";
    
    	$sql.= " WHERE s.entity IN (".getEntity('resource', 1).")";
    	if ($fk_place > 0) $sql.= " AND s.fk_place=".$fk_place;
    	...
    	if ($resql)
    	{
    		if ($conf->use_javascript_ajax && $conf->global->RESSOURCE_BUILDING_USE_SEARCH_TO_SELECT && ! $forcecombo && ! $options_only)
    		{
    			$out.= ajax_combobox($htmlname, $event, $conf->global->RESSOURCE_BUILDING_USE_SEARCH_TO_SELECT);
    		}
